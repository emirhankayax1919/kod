...
bist100 = yf.download('^XU100', start='2010-01-01').reset_index()

# Verileri ön işleme tabi tutuyoruz
scaler = MinMaxScaler(feature_range=(0, 1))
scaled_data = scaler.fit_transform(bist100['Close'].values.reshape(-1, 1))
...
y_pred = pd.DataFrame(y_pred, index=valid.index, columns=['Predictions'])
valid = pd.concat([valid, y_pred], axis=1)
...
def calculate_indicators(data, talib=None):
